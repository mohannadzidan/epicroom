...
if [ -z "$1" ] || [ -z "$2" ]; then
    echo "Usage: $0 <directory> <HEADER_FILE>"
    exit 1
fi
...
# Output header file (configured via second CLI argument)
HEADER_FILE="$2"
SRC_FILE="$2.c"
...
declare -a files
declare -a files_gzibbed_content
declare -a files_gzibbed_content_lengths
...
process_directory() {
    ...
    for entry in "$dir"/*; do
        if [ -d "$entry" ]; then
            ...
            process_directory "$entry" "$relative_path/$(basename "$entry")"
        elif [ -f "$entry" ]; then
            # If it's a file, process it
            local relative_file_path="${relative_path}/$(basename "$entry")"
            ...
            local gzipped_length
            gzipped_length=$(wc -c <"$gzipped_temp_file")

            # Convert the gzipped data to a C array using xxd
            local gzipped_content
            gzipped_content=$(xxd -i <"$gzipped_temp_file")
            ...
            {
                echo "// Original path: $relative_file_path"
            ...
            files_gzibbed_content+=("$array_name")
            files_gzibbed_content_lengths+=("$gzipped_length")
...
# Write the `files_count` constant
{
...
# Write the `files_gzibbed_content` array
{
...

{
    ...
    for content in "${files_gzibbed_content[@]}"; do
        echo "    $content,"
    done
    echo "};"
    echo ""
...
# Write the `files_gzibbed_content_lengths` array
total_size=0
...

{
    ...
    for length in "${files_gzibbed_content_lengths[@]}"; do
...
echo "Header file generated at: $SRC_FILE"